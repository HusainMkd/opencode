          ...
          Effect.gen(function* () {
            ...
            if (summary.info.role === "assistant" && summary.info.tokens) {
              ...
              sessionStats.cacheTokens +=
                (summary.info.tokens.cache?.read || 0) + (summary.info.tokens.cache?.write || 0)
      ...
      function footer(opts?: { image?: boolean }) {
        ...
        const stats =
          sessionStats.requests > 0
            ? `<sub>📊 ${formatTokenCount(sessionStats.inputTokens)} in / ${formatTokenCount(sessionStats.outputTokens)} out | $${sessionStats.cost.toFixed(3)} | ${sessionStats.requests} req</sub>\n\n`
            : ""
        return `\n\n${image}${shareUrl}${stats}[github run](${runUrl})`
      }
